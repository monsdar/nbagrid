...
class GameResultTests(TestCase):
        ...
        for i, player in enumerate(players):
            for j in range(pick_counts[i]):
                GameResult.objects.create(
                    date=self.yesterday - timedelta(days=j), cell_key="0_0", player=player, guess_count=1
                )

        # Initialize scores for today
        game_factor = 5
        GameResult.initialize_scores_from_recent_games(date=self.today, cell_key="0_0", game_factor=game_factor)

        # Get all results for today
        results = GameResult.objects.filter(date=self.today, cell_key="0_0").order_by("-guess_count")
    ...
    def test_initialize_scores_with_filters(self):
        ...
        GameResult.initialize_scores_from_recent_games(
            date=self.today, cell_key="0_0", game_factor=5, filters=[PositionFilter("PG")]
        )

        # Verify only PG players have entries
        results = GameResult.objects.filter(date=self.today, cell_key="0_0")

        self.assertEqual(results.count(), 1)  # Only one PG player
        self.assertEqual(results.first().player, players[0])  # Player0 is PG
        self.assertEqual(
            results.first().guess_count, 0
        )  # guess_count 0, because only the PG is picked and this is in the bottom third
