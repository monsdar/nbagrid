...
class GameCompletionTests(TestCase):
    def setUp(self):
        ...
        GameCompletion.objects.create(
            date=self.today, session_key=self.session1, correct_cells=9, final_score=8.9, completion_streak=3, perfect_streak=3
        )
        ...
        GameCompletion.objects.create(
            date=self.today, session_key=self.session2, correct_cells=9, final_score=8.2, completion_streak=2, perfect_streak=2
        )

        # Session 3: 1-day streak (just today)
        GameCompletion.objects.create(
            date=self.today, session_key=self.session3, correct_cells=9, final_score=7.5, completion_streak=1, perfect_streak=1
        )
    ...
    def test_get_ranking_with_neighbors(self):
        ...
        GameCompletion.objects.create(
            date=self.today, session_key=self.session2, correct_cells=9, final_score=15, completion_streak=2, perfect_streak=2
        )
        GameCompletion.objects.create(
            date=self.today, session_key=self.session3, correct_cells=9, final_score=10, completion_streak=1, perfect_streak=1
        )

        # Create completions with different scores
        GameCompletion.objects.create(
            date=self.today, session_key=session4, correct_cells=9, final_score=9.0, completion_streak=1, perfect_streak=1
        )
        GameCompletion.objects.create(
            date=self.today, session_key=session5, correct_cells=9, final_score=8.0, completion_streak=1, perfect_streak=1
        )
        GameCompletion.objects.create(
            date=self.today, session_key=session6, correct_cells=9, final_score=7.0, completion_streak=1, perfect_streak=1
        )
        GameCompletion.objects.create(
            date=self.today,
